...
def test_point_process_multi_target_tracker_detector_context(detector, predictor):
    ...
    context_hypothesiser = ContextHypothesiser(hypothesiser)
    reducer = GaussianMixtureReducer(prune_threshold=1e-5, merge_threshold=4)
    phd_updater = PHDUpdater(updater=updater, prob_detection=0.1)

    tracker = PointProcessMultiTargetTracker(
        detector=context_detector,
        updater=phd_updater,
        hypothesiser=context_hypothesiser,
        reducer=reducer,
        birth_component=birth_component
        )
    ...
    assert tracks
    assert context_hypothesiser.detector_contexts == [detector_context]
